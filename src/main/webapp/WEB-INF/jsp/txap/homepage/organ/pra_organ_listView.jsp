				...
				<div class="widget-body-toolbar bg-color-white">
					<div class="row">
						<div class="col-sm-12 col-md-12 text-align-right">
							...
							<div class="btn-group">  
								<button class="btn btn-sm btn-success" type="button" onclick="location.href='/txap/homepage/org/excelDownload.do'">
...
function pf_insertPersonInfo(){
	...
	$('#orgPersonForm').attr('action','/txap/homepage/org/person/insert.do');
	$('#orgPersonForm').submit();
...
function pf_insertOrgInfo(){
	...
	$('#orgInfoForm').attr('action','/txap/homepage/org/insert.do');
	$('#orgInfoForm').submit();
...
function pf_updateOrgInfo(){
	...
	$('#DN_HOMEDIV_C').val($('#MN_HOMEDIV_C').val())
	$('#orgInfoForm').attr('action','/txap/homepage/org/update.do');
...
function pf_deleteOrgInfo(){
	...
	$('#DN_KEYNO').val(code);
	$('#DN_HOMEDIV_C').val($('#MN_HOMEDIV_C').val())
	$('#orgInfoForm').attr('action','/txap/homepage/org/delete.do');
	$('#orgInfoForm').submit();
}
...
function pf_detailOrg(key){
	...
	$.ajax({
		type: "POST",
		url: "/txap/homepage/org/dataAjax.do",
		data: {"key":key,
...
/* 					if(data.DN_KEYNO == order[i].DN_KEYNO){
			...
			$(".orgInfoTable tbody .order").find("select[name='DN_LEV_AFTER']").on('change',function(){
				...
				$.ajax({
					type: "POST",
					url: "/txap/homepage/org/person/chageLev2.do",
					data: {
						"DN_KEYNO":dnkey,
					...
					success : function(data){
						location.href="/txap/homepage/org/org.do?key="+key+"&MN_HOMEDIV_C="+$('#MN_HOMEDIV_C').val();
					},
					error: function(){
...
function pf_changeLev(){
	...
	$.ajax({
		type: "POST",
		url: "/txap/homepage/org/person/chageLev.do",
...
function pf_excel(){
	...
	$("#menuForm").attr("action", "/txap/homepage/org/excelAjax.do");
	$("#menuForm").submit();
}
    ...
    if("xlsx".indexOf(ext) != -1){
       if(confirm('조직도를 일괄등록 하시겠습니까?\n모든 데이터가 초기화됩니다.')){
            ...
            setTimeout(function () {
                $.ajax({
                    type: "POST",
                    url: '/txap/homepage/org/excelInsertAjax.do',
                    async: false,
                    processData: false,
                    contentType: false,
                    ...
                    success : function(data){
                        ...
                        location.href='/txap/homepage/org/org.do';
                    }, 
                    error: function(){
