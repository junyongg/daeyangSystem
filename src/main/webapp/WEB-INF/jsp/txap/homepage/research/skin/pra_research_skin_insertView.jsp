...
function pf_researchSkinInsert(){
	...
	if(confirm("페이지평가 스킨을 생성하시겠습니까?")){
		cf_replaceTrim($("#Form"));
		$("#Form").attr("action", "/txap/homepage/research/insert.do");
		$("#Form").submit();
	}
...
function pf_researchSkinUpdate(){
	...
	if(confirm("페이지평가 스킨을  수정하시겠습니까?")){
		cf_replaceTrim($("#Form"));
		$("#Form").attr("action", "/txap/homepage/research/update.do");
		$("#Form").submit();
	}
...
function pf_skinDelete() {
	var keyno = '${PRS_DATA.PRS_KEYNO }';
 	...
 	$.ajax({
		type: "POST",
		url: "/txap/homepage/research/useSkinAjax.do",
		data : {
			 "PRS_KEYNO" : keyno
		},
...
function pf_researchSkinDelete(){
		cf_replaceTrim($("#Form"));
		$("#Form").attr("action", "/txap/homepage/research/delete.do");
		$("#Form").submit();
}
...
function pf_data(){	
	...
	$.ajax({
		type: "POST",
		url: "/txap/homepage/research/skindataAjax.do",
		data : {
				"PRS_KEYNO" : keyno
				},
...
function pf_introUse(PRSH_KEYNO){
	...
	setTimeout(function(){
		$.ajax({
			type: "POST",
			url: "/txap/homepage/research/returnPageAjax.do",
...
function pf_compareData(PRSH_KEY, PRS_KEY){
	...
	$.ajax({
		type: "POST",
		url: "/txap/homepage/research/compareAjax.do",
