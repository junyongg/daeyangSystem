...
function pf_boardHtmlInsert(){
	...
	$("#Form").attr("action","/txap/homepage/board/data/html/insert.do?${_csrf.parameterName}=${_csrf.token}");
	$("#Form").submit();
}
function pf_boardHtmlUpdate(){
	//웹에디터 내용 textarea로 복사
	editor_object.getById["BIH_CONTENTS"].exec("UPDATE_CONTENTS_FIELD", []);
	$("#Form").attr("action","/txap/homepage/board/data/html/update.do?${_csrf.parameterName}=${_csrf.token}");
	$("#Form").submit();
}
function pf_boardHtmlDelete(useyn){
	$("#BIH_USE_YN").val(useyn)
	$("#Form").attr("action","/txap/homepage/board/data/html/delete.do?${_csrf.parameterName}=${_csrf.token}");
	$("#Form").submit();
}
function pf_back(){
	$("#Form").attr("action","/txap/homepage/board/dataView.do?${_csrf.parameterName}=${_csrf.token}");
	$("#Form").submit();
}
</script>
<style>
