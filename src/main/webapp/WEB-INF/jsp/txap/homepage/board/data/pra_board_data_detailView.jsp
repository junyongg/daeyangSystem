...
function pf_UpdateMove(keyno){
	$("#actionForm").attr("action", "/txap/homepage/board/data/actionView.do");
	$("#actionForm").submit();
}
// 리스트로 돌아가기
function pf_back(){
	$("#actionForm").attr("action", "/txap/homepage/board/dataView.do");
	$("#actionForm").submit();
}
...
function pf_deleteMove(del,bnkey){
	...
	if(confirm(msg)){
		...
		if(('${BoardType.BT_DEL_MANAGE_YN}' == 'Y' && del == 'Y' && '${BoardType.BT_DEL_POLICY}' == 'L')){
			$('#actionForm').attr('action','/txap/homepage/board/'+bnkey+'/deleteView.do');
		}else{
			$("#actionForm").attr("action", "/txap/homepage/board/data/state.do");
		}
		$("#actionForm").submit();
	}
}
...
function pf_deleteComp(bnkey){
	if(confirm("정말 삭제하시겠습니까?")){
		$("#actionForm").attr("action", "/txap/homepage/board/data/state.do?delComplete=true");
		$("#actionForm").submit();
	}
}
...
function pf_commentDelete(bc_keyno){
	if(confirm("삭제 하시겠습니까?")== true){
		$.ajax({
		    type   : "post",
		    url    : "/txap/homepage/board/data/comment/deleteAjax.do",
...
function pf_DetailMove(keyno){
	$('#BN_KEYNO').val(keyno);
	$("#actionForm").attr("action", "/txap/homepage/board/data/detailView.do");
	$("#actionForm").submit();
}
...
function pf_moveBoard(keyno){
	...
	$.ajax({
	    type   : "post",
	    url    : "/txap/homepage/board/moveCheckAjax.do",
	    data   : {"BN_KEYNO" : keyno},
	    async  : false,
	...
	if(check){
		$("#actionForm").attr("action", "/txap/homepage/board/moveView.do");
		$("#actionForm").submit();		
	}else{
										...
										<form:form id="Form" method="post">
											...
											<div class="table-responsive">
												<jsp:include page="/WEB-INF/jsp/txap/include/search/pra_search_header_paging.jsp" flush="true">
													<jsp:param value="/txap/homepage/board/data/detailView/comment/pagingAjax.do" name="pagingDataUrl" />
													<jsp:param value="/txap/homepage/board/data/detailView/comment/excelAjax.do" name="excelDataUrl" />
												</jsp:include>
...
function pf_compareData(KEY1, KEY2, KEY3){
	$.ajax({
		type: "POST",
		url: "/txap/homepage/board/data/detailView/compareAjax.do",
...
function pf_introUse(key1, key2){
	$("#actionForm").attr("action", "/txap/homepage/board/data/restore.do?bnh_keyno="+key1+"&bn_keyno="+key2);
	$("#actionForm").submit();
}

//내용보기
function pf_introPreview(KEYNO){
	window.open('/txap/homepage/board/data/detailView/historyContents.do?BNH_KEYNO='+KEYNO,"","width=960, height=800")
