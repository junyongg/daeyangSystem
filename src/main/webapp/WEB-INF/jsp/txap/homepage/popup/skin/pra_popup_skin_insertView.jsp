...
function pf_popupSkinInsert(){
	...
	if(confirm("팝업 스킨을 생성하시겠습니까?")){
		cf_replaceTrim($("#Form"));
		$("#Form").attr("action", "/txap/homepage/popupSkin/insert.do");
		$("#Form").submit();
	}
...
function pf_popupSkinUpdate(){
	...
	if(confirm("팝업 스킨을  수정하시겠습니까?")){
		cf_replaceTrim($("#Form"));
		$("#Form").attr("action", "/txap/homepage/popupSkin/update.do");
		$("#Form").submit();
	}
...
function pf_skinDelete() {
	var keyno = '${PIS_DATA.PIS_KEYNO }';
 	...
 	$.ajax({
		type: "POST",
		url: "/txap/homepage/popupSkin/useSkinAjax.do",
		data : {
			 "PIS_KEYNO" : keyno
		},
...
function pf_popupSkinDelete(){
		cf_replaceTrim($("#Form"));
		$("#Form").attr("action", "/txap/homepage/popupSkin/delete.do");
		$("#Form").submit();
}
...
function pf_data(){	
	...
	$.ajax({
		type: "POST",
		url: "/txap/homepage/popupSkin/skindataAjax.do",
		data : {
				"PIS_KEYNO" : keyno
				},
...
function pf_introUse(PISH_KEYNO){
	...
	setTimeout(function(){
		$.ajax({
			type: "POST",
			url: "/txap/homepage/popupSkin/returnPageAjax.do",
...
function pf_compareData(PISH_KEY, PIS_KEY){
	...
	$.ajax({
		type: "POST",
		url: "/txap/homepage/popupSkin/compareAjax.do",
