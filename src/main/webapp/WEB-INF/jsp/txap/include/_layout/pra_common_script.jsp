...
function pf_dataHistory(rmKey){
	...
	if(rmKey){
		$.ajax({
			type: "POST",
			url: "/txap/homepage/common/"+resourceType+"/dataHistoryAjax.do", 
			data : {"RM_KEYNO"	: rmKey},
...
function pf_introUse(RMH_KEYNO){
	...
	setTimeout(function(){
		$.ajax({
			type: "POST",
			url: "/txap/homepage/common/recovery/dataReturnAjax.do",
...
function pf_compareData(RMH_KEY1, RM_KEY2){
	...
	$.ajax({
		type: "POST",
		url: "/txap/homepage/common/compare/dataCompareAjax.do",
	...
	if(key){
		if(confirm('삭제하시겠습니까?')){
			...
			setTimeout(function(){
				$.ajax({
					type: "POST",
					url: "/txap/homepage/common/"+resourceType+"/DeleteAjax.do",
					async:false,
					data : {
							"RM_KEYNO":key,
							"RM_ORDER":order,
