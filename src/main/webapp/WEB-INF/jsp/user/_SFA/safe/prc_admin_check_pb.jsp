   ...
   <div class="container grid px-6 mx-auto">
     <div class="relative">
       ...
       <div class="w-full rounded-lg bg-white p-5 my-4 md:md-4 lg:my-7">
           ...
           <div class="mt-5 mb-3 flex flex-wrap">
           		 <label for="area_0" class="text-black bg-badge-blue py-2 px-3 rounded-full mx-2 mb-2 text-xs cursor-pointer flex-shrink-0">지역 전체</label>
             	 <input type="checkbox" name="area_0" id="area_0" value ="20"  ${search.orderCondition eq '10'? 'selected':'' }  onclick = "pf_searchDate(this.value)" class="hidden month_btn" checked>
	             
	             <c:forEach items="${map }" varStatus="status" var="area">
	             		<label for="area_${status.count }" class="text-black bg-badge-blue py-2 px-3 rounded-full mx-2 mb-2 text-xs cursor-pointer flex-shrink-0">${map[status.index].SU_SA_AREA}</label>
             			<input type="checkbox" name="area_${status.count }" id="area_${status.count }" value ="${status.count + 20}"  ${search.orderCondition eq '13'? 'selected':'' }  onclick = "pf_searchDate(this.value)" class="hidden month_btn">
	             </c:forEach>
           </div>
       </div>
       <div class="w-full rounded-lg bg-white p-5 my-4 md:md-4 lg:my-7">
