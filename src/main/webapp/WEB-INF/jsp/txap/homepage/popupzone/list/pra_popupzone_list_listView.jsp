						...
						<div class="widget-body " >
							...
							<div class="table-responsive">
								<jsp:include page="/WEB-INF/jsp/txap/include/search/pra_search_header_paging.jsp" flush="true">
									<jsp:param value="/txap/homepage/popupzone/list/pagingAjax.do" name="pagingDataUrl" />
									<jsp:param value="/txap/homepage/board/typeView/excelAjax.do" name="excelDataUrl" />
								</jsp:include>
...
function pf_categoryUpdate(key) {
	$("#TLM_KEYNO").val(key);
	$("#Form").attr("action", "/txap/homepage/popupzone/list/updateView.do");
	$("#Form").submit();
} 

//리스트 등록페이지 이동 
function pf_listRegist() {
	$("#Form").attr("action", "/txap/homepage/popupzone/list/insertView.do");
	$("#Form").submit();
}
...
function pf_useList(key, use) {
	$('#TLM_KEYNO').val(key);
	$('#TLM_USE_YN').val(use);
	$('#Form').attr('action','/txap/homepage/popupzone/list/check.do');
	$('#Form').submit()
}
...
function fn_updateLevel(tlm_keyno, tlm_tcgm_keyno, beforeOrder, order){
	...
	$("#TLM_ORDER_BEFORE").val(beforeOrder);
	$("#TLM_ORDER").val(order);
	$("#Form").attr("action", "/txap/homepage/popupzone/list/orderUpdate.do");
	$("#Form").submit();
}
