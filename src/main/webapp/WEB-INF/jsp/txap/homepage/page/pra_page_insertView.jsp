...
function set_contentsData(){
	...
	if(${not empty HTMLViewData.MVD_EDITOR_TYPE}){	//데이터 있을 경우에 컨텐츠 넣음
		...
		$.ajax({
	        type: "POST",
	        url: "/txap/homepage/page/DataViewAjax.do",
	        async:false,
...
function pf_htmHistoryData(key){
	if(key){
		...
		$.ajax({
	         type: "POST",
	         url: "/txap/homepage/page/history/dataAjax.do",
	         data : {"MVD_KEYNO":key},
...
function pf_checkAjax(){
	...
	setTimeout(function(){
		   $.ajax({
	            type: "POST",
	            url: "/txap/homepage/page/versionCheckAjax.do",
	            data : $("#Form").serialize(),
	            success : function(data){
...
function success(){
	...
	$.ajax({
		type: "post",
		url: "/txap/homepage/page/insertAjax.do?${_csrf.parameterName}=${_csrf.token}",
		data: data,
		success : function(data){
			$("#page_ch").html(data);
		},
...
<%@ include file="/WEB-INF/jsp/txap/include/_layout/pra_editor_imgSetting.jsp" %>    